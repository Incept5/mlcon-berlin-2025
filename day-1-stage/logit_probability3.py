...
# Constants
DEFAULT_MODEL_PATH = "/Users/jdavies/.cache/lm-studio/models/tensorblock/unsloth_Qwen3-30B-A3B-Instruct-2507-GGUF/Qwen3-30B-A3B-Instruct-2507-Q4_K_S.gguf"
...
class TokenProbabilityAnalyzer:
    ...
    def analyze_continuation(self, prompt: str, config: InferenceConfig) -> List[Tuple[str, float, float]]:
        ...
        # BUT only if finish_reason is "length" (not "stop" which means legitimate end)
        if not result and choice and choice.get("finish_reason") == "length":
            self.debug_info += "\nAutomatically retrying due to empty logprobs bug...\n"
            ...
            if "choices" in output2 and output2["choices"]:
                ...
                if isinstance(logprobs2, dict):
                    ...
                    if top_logprobs2:
                        self.debug_info += f"✅ Auto-retry: Successfully recovered from empty logprobs bug\n"
                        result = self._extract_probabilities(output2, config)
                    else:
                        self.debug_info += "Retry also returned empty logprobs\n"
    ...
    def _extract_probabilities(self, output: dict, config: InferenceConfig) -> List[Tuple[str, float, float]]:
        ...
        if finish_reason == "stop":
            self.debug_info += "🛑 Model reached stop token - generation complete\n"
            return []

        logprobs_data = choice.get("logprobs", {})

        if not logprobs_data:
            self.debug_info += f"ERROR: No logprobs data returned (finish_reason: {finish_reason})\n"
            return []

        top_logprobs = logprobs_data.get("top_logprobs", [])

        if not top_logprobs:
            self.debug_info += f"ERROR: Empty top_logprobs list (finish_reason: {finish_reason})\n"
            if finish_reason == "length":
                self.debug_info += "This is the intermittent llama-cpp bug - will attempt retry\n"
            return []
...
def analyze_tokens(mode: str, system_prompt: str, user_prompt: str, generate_prompt: str,
    ...
    try:
        ...
        if not token_probs:
            ...
            # Check if this was a legitimate stop
            if "🛑 Model reached stop token" in current_analyzer.debug_info:
                debug_msg = "**✅ Generation Complete**\n\n"
                debug_msg += "The model has reached a natural stopping point (end-of-sequence token).\n\n"
...
def add_top_token(mode: str, system_prompt: str, user_prompt: str, generate_prompt: str,
    ...
    try:
        ...
        if not token_probs:
            # Check if this was a legitimate stop
            if "🛑 Model reached stop token" in current_analyzer.debug_info:
                formatted_results = "**✅ Generation Complete**\n\n"
                formatted_results += "The model has reached a natural stopping point (end-of-sequence token).\n\n"
