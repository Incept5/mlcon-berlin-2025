"""RAG (Retrieval-Augmented Generation) for Grimm Fairy Tales using Groq and SentenceTransformers.
...
2. Generates embeddings using SentenceTransformers (local, free)
3. Stores embeddings in memory (no persistence)
4. Retrieves relevant chunks using cosine similarity
5. Generates answers using Groq's LLM API
...
# Configuration
EMBEDDING_MODEL = "all-MiniLM-L6-v2"  # Fast, efficient sentence embeddings (384 dimensions)
LLM_MODEL = "llama-3.3-70b-versatile"  # Groq's powerful LLM for text generation
...
class EmbeddingClient:
    ...
    def get_embedding(self, text: str) -> np.ndarray:
        ...
        """
        # SentenceTransformers returns a 2D array, we want 1D
...
class GrimmRAG:
    ...
    This implementation:
    - Uses SentenceTransformers for embeddings (local, free)
    - Uses Groq for text generation (fast, API-based)
...
def main():
    """Demo the Grimm fairy tales RAG system with Groq.
    ...
    print("  - Grimm fairy tales text file\n")
    ...
    try:
        ...
        if not fairy_tale_file:
            ...
            print("Search paths:", [str(p) for p in search_paths])
            return
        ...
        print("\nPlease ensure:")
        print("  1. You have a .env file with GROQ_API_KEY")
        print("  2. Or set the environment variable: export GROQ_API_KEY='your-key-here'")
    except Exception as e:
        print(f"\nError: {e}")
        import traceback
        traceback.print_exc()
