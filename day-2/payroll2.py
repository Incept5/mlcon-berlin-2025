"""LA City Payroll SQL Query Generator using Ollama LLM

This script demonstrates text-to-SQL capabilities using a local LLM (Ollama).
It retrieves the database schema and sample data, sends them to an LLM to generate
...
3. Send schema + samples to Ollama with natural language prompt
...
- qwen3-vl:4b-instruct model installed in Ollama
...
def query_ollama(schema, sample_data=None):
    """Send database schema and natural language query to Ollama LLM for SQL generation.
    ...
    API Configuration:
    ------------------
    - Model: qwen3-vl:4b-instruct (vision-language model, good for structured data)
    - Context window: 32768 tokens (allows large schemas and samples)
...
average hourly rate for people working in the LAPD for each year\n\n{schema}"""
    ...
    response = requests.post(
        ...
        json={
            "model": "qwen3-vl:4b-instruct",  # Vision-language model suitable for structured data
            "prompt": prompt,
            "stream": False,  # Wait for complete response (not streaming chunks)
            "options": {
                "num_ctx": 32768,  # Large context window for schema + samples
...
def main():
    """Main execution function demonstrating text-to-SQL with Ollama.
    ...
    # Step 3: Send schema and samples to Ollama for SQL generation
